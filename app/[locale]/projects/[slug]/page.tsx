...
export default async function CasePage({params}:{params:Promise<{locale:string;slug:string}>}){
  ...
  return <><SiteHeader locale={locale} path={`/projects/${slug}`}/><main className="case-main">
    <section className={`case-hero ${isRescue?"case-purple":"case-blue"}`}><div className="shell"><Link className="back-link" href={`/${locale}`}>← {t.back}</Link><div className="case-title"><p className="eyebrow">{item.status}</p><h1>{item.title}</h1><p className="case-lead">{item.lead}</p><p>{item.summary}</p>{"live" in item&&<div className="case-actions"><a className="button button-primary" href={item.live} target="_blank" rel="noreferrer">{t.live} ↗</a><div><span className="code-label">{t.code}</span><CopyCode code={item.access} hint={t.copyHint}/></div></div>}</div><div className="case-shot"><Image src={item.image} alt={`${item.title} product interface`} fill priority sizes="(max-width: 900px) 94vw, 55vw"/></div></div></section>
    <section className="case-body shell"><aside><p className="eyebrow">{locale==="zh"?"CASE AT A GLANCE":"CASE AT A GLANCE"}</p>{item.metrics.map(([value,label])=><div className="metric" key={label}><strong>{value}</strong><span>{label}</span></div>)}</aside><div className="case-sections">{item.sections.map(([title,body],i)=><article key={title}><span>0{i+1}</span><div><h2>{title}</h2><p>{body}</p></div></article>)}</div></section>
